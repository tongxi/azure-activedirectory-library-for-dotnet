    ...
    class AzureBlobTokenCache3 : TokenCache
    {
        ...
        void CustomTokenCache_AfterAccess(TokenCacheAccessArgs args)
        {
            if (this.HasStateChanged)
            {
                ...
                {
                    byte[] remoteBlob = new byte[blockBlob.Properties.Length];
                    blockBlob.DownloadToByteArray(remoteBlob, 0, AccessCondition.GenerateLeaseCondition(leaseId));
                    this.MergeStates(remoteBlob);
                }
